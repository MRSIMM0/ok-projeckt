...
function main(data){
    ...
    max_x = Math.max(...arr.map((item)=> item[1]));
    min_x = Math.min(...arr.map((item)=> item[1]));

    range_x = max_x - min_x;

    max_y = Math.max(...arr.map((item)=> item[2]));
    min_y = Math.min(...arr.map((item)=> item[2]));

    range_y = max_y - min_y

    normalized = arr.map((item)=> {
        return [(item[1]-min_x)/range_x ,(item[2]-min_y)/range_y ]
    } )
...
function drawPoints(normalized,ctx){
    normalized.forEach((x) =>{
        ...
        ctx.beginPath();
        ctx.arc(X, Y, R, 0, 10 * Math.PI, false);
        ctx.lineWidth = 4;
